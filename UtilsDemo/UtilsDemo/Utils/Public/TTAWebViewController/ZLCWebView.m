...
@interface ZLCWebView ()<UIAlertViewDelegate, WKScriptMessageHandler>
...
- (instancetype)initWithFrame:(CGRect)frame jsCalledMessages:(NSArray <NSString *>*)messages {
    if (self = [super initWithFrame:frame]) {
        if(isiOS8) {
            ...
            [messages enumerateObjectsUsingBlock:^(NSString * _Nonnull message, NSUInteger idx, BOOL * _Nonnull stop) {
                [userController addScriptMessageHandler:weakDelegate name:message];
            }];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([self.delegate respondsToSelector:@selector(zlcwebViewDidReceivedJSMessage:)]) {
        [self.delegate zlcwebViewDidReceivedJSMessage:message];
    }
